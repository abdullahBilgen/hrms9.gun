...
import kodlamaio.Hrms.core.UserLastIndexService;
...
public class CandidateManager implements CandidateService {
	...
	private UserLastIndexService userLastIndexService;

	@Autowired
	public CandidateManager(CandidateDao candidateDao, UserDao userDao,
			CandidateValidatorService candidateValidatorService, MailValidationService mailValidationService,
			UserLastIndexService userLastIndexService) {
		super();
		this.candidateDao = candidateDao;
		this.userDao = userDao;
		this.candidateValidatorService = candidateValidatorService;
		this.mailValidationService = mailValidationService;
		this.userLastIndexService = userLastIndexService;
	}
	...
	public Result add(Candidate candidate) {
		...
		candidate.getUser().setUserId(lastIndex(new User())+1);
	...
	private int lastIndex(User user) {
		
		return this.userLastIndexService.lastIndex(user);
	}
	private boolean dataControl(Candidate candidate) {
		if (candidate.getUser().getEmail() == null || candidate.getUser().getEmail().isBlank()
				|| candidate.getFirstName() == null || candidate.getFirstName().isBlank()
				|| candidate.getLastName() == null || candidate.getLastName().isBlank()
				|| candidate.getIdentityNumber() == null || candidate.getIdentityNumber().isBlank()
				|| candidate.getUser().getPassword() == null || candidate.getUser().getPassword().isBlank()
	...
	public DataResult<List<Candidate>> findByCandidateIdOrderBySchoolsGraduationDate(int candidateId) {
		return new SuccessDataResult<List<Candidate>>(
				this.candidateDao.findByCandidateIdOrderBySchools_graduationDate(candidateId));
	}

	@Override
	public DataResult<List<Candidate>> findByCandidateIdOrderByJobExperiencesEndDate(int candidateId) {
		return new SuccessDataResult<List<Candidate>>(
				this.candidateDao.findByCandidateIdOrderByJobExperiences_endDate(candidateId));
	}
